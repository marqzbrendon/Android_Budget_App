...
class AppDatabase {
    ...
    fun editExpenseDb(db: FirebaseFirestore, data: AppData, index: Int, newAppData: Expense) {
        ...
        docRef.update("source", newAppData.source, "value", newAppData.value, "category", newAppData.categoryName)
...
//    }
    ...
    fun retrieveAllDocuments(db: FirebaseFirestore?, data: AppData) {
        println("made it to retrieve all")
        db?.collection("${data.year}")?.document("${data.month}")?.collection("income")
            ?.addSnapshotListener(object : EventListener<QuerySnapshot?> {
                override fun onEvent(snapshots: QuerySnapshot?, e: FirebaseFirestoreException?) {
                    ...
                    if (snapshots != null) {
                        ...
                        println("myincomes ${data.incomesDb}")
                    }
                }
            })
